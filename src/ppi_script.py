...
class PPI_Network:
    ...
    def get_hub_gene(self, ppi_graph):
        """Identify hub genes as nodes with degree > mean degree."""
        logger = logging.getLogger()
    ...
    def extract_hub_subgraph(self, ppi_graph, hub_genes):
        """Return subgraph with hub genes and their neighbors."""
    ...
    def render_network(self, ppi_graph, imp_genes, html_file, title="Protein–Protein Interaction (PPI) Network"):
        """Render PyVis network: hub genes red, others blue, uniform shape and size, with legend and title."""
        logger = logging.getLogger()
        if not ppi_graph.nodes():
        ...
        # Add nodes (all same shape and size)
        for node_id in ppi_graph.nodes():
            if node_id in imp_genes:  # hub gene
        ...
        # Add edges - plain gray thin lines
...
def PPI_Analysis(gene_csv_file, min_conf=700, hub_only=True):
    ...
    # Use hub subgraph if hub_only=True
