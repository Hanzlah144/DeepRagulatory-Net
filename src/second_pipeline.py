...
class SecondPipeline:
    def __init__(self, circ_file, mirna_file, deg_file, model_file, encoder_file, scaler_file, temp_dir, output_dir):
    ...
    def create_comprehensive_excel(self):
        """
        Create a comprehensive Excel file containing overlapping genes with their 
        interacting miRNAs and circRNAs.
        """
        logger = logging.getLogger()
        
        try:
            # Read overlapping mRNAs data
            ...
            for _, row in overlap_df.iterrows():
                ...
                # Get circRNAs that interact with this miRNA
                circrnas = circ_mirna_map.get(mirna, [])
                
                if circrnas:
                    # Create separate rows for each circRNA
                    ...
                    # miRNA has no circRNA interactions
                    comprehensive_data.append({
                        'Gene': gene,
                        'miRNA': mirna,
            ...
            # Sort by gene, then miRNA, then circRNA
            ...
            # Create Excel file without formatting first
            ...
            # Apply formatting separately after the file is created
            try:
