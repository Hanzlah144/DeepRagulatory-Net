...
# ✅ Optimized: Parallel miRNA target extraction with caching and session reuse
def query_mirdb_optimized(mirna_name, session=None, max_retries=2, retry_delay=1):
    """Optimized miRDB query with caching and session reuse"""
    # Check cache first
...
# ⬇ Unchanged: DEG–miRNA overlap + Venn diagram
def overlap_mrnas(deg_file, matches_dir='my_output', output_dir='.'):
    ...
    # Parallel processing of miRNA queries with session reuse
